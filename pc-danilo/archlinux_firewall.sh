...
func_HOST_FTP() {
  ...
  $CMD_IPTABLES_V4 -A INPUT -p tcp --sport 21 -i $INTERFACE_LAN --dport $PORTAS_ALTAS -j DROP  
}
...
func_OUTPUTCHAIN_ALLOW() {
  ...
  $CMD_IPTABLES_V4 -A OUTPUT -p tcp -m tcp --dport 2083 -m comment --comment "Hostgator" -j ACCEPT  
  $CMD_IPTABLES_V4 -A OUTPUT -p tcp -m tcp --dport 5901 -m comment --comment "XRDP" -j ACCEPT  
